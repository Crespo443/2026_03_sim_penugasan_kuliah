<x-layouts.app title="Kalender Akademik">
    ...
    <div x-data="kalenderApp()" x-init="init()">
        ...
        <x-ui.card class="mb-6">
            ...
            {{-- Legend --}}
            <div class="flex items-center gap-4 mt-3 text-xs md:text-sm">
        ...
        <x-ui.card>
            ...
            <div
                ...
                <template x-for="(cell, index) in calendarCells" :key="index">
                    <div class="bg-base-100 min-h-[60px] sm:min-h-[100px] p-1 sm:p-1.5 relative group focus-within:z-30 cursor-pointer" tabindex="0"
                        :class="{
                            'bg-primary/5': cell.isToday,
                            'opacity-40': !cell.currentMonth
                        }"
                        @click="showEventModal(cell, index)">
                        <div class="font-medium mb-1 flex items-center gap-1"
                            :class="cell.isToday ? 'text-primary font-bold' : 'text-base-content/70'">
                            <span x-text="cell.day"></span>
                            <span x-show="cell.isToday" class="hidden md:block badge badge-primary badge-xs">Hari Ini</span>
                        </div>
            ...
            {{-- Modal Event --}}
            <x-ui.modal id="modal-event" size="sm" :closeButton="true">
                <x-slot:title>
                    <h3 class="font-bold text-lg">Event Tanggal <span x-text="modalDate"></span></h3>
                </x-slot:title>
                <div x-show="modalEvents.length === 0" class="text-center text-base-content/50 text-xs">Tidak ada event</div>
                <template x-for="event in modalEvents" :key="event.title">
                    <div class="mb-2 flex items-center gap-2">
                        <span class="inline-block w-2 h-2 rounded-full"
                            :class="event.type === 'deadline' ? 'bg-error' : 'bg-primary'"></span>
                        <span class="truncate text-xs font-medium" :class="event.type === 'deadline' ? 'text-error' : 'text-primary'" x-text="event.title"></span>
                    </div>
                </template>
            </x-ui.modal>
        </x-ui.card>
        ...
        <script>
            function kalenderApp() {
                return {
                    ...
                    showEventModal(cell, index) {
                        this.modalEvents = cell.events;
                        this.modalDate = cell.day;
                        setTimeout(() => {
