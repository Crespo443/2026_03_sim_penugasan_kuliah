<x-layouts.app title="Edit Tugas">
    ...
    <x-ui.card class="">
        <form method="POST" action="{{ route('tugas.update', $tugas->id) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <x-ui.select name="prioritas" label="Prioritas" :required="true" placeholder="Pilih prioritas"
                    ...
                    :value="$tugas->prioritas" />
                <x-ui.input name="file" label="Upload File (PDF/IMG)" type="file" accept="application/pdf,image/*" :value="$tugas->file"/>
            </div>

            <x-ui.textarea name="catatan" label="Catatan" placeholder="Catatan tambahan (opsional)" :value="$tugas->catatan" />
